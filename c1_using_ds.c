...
// Miller-Rabin primality test using an array to store results
int miller_rabin(unsigned long n, int k) {
    ...
    // Create an array to store bases and their corresponding results
    unsigned long bases[k];
    unsigned long results[k];
    ...
    // Output all tested bases and results
    for (int i = 0; i < k; i++) {
        printf("Base %lu gave result %lu\n", bases[i], results[i]);
    }
